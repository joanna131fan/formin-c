...
% This is currently only set up to read files with CD3Zeta parameters - not
% CD3 Epsilon
...
model = 3; % 1 - EB0 sweep, 2 - EP0 Sweep
...
%controlSubfolder = '20180306CD3ZetaElectrostaticsEB0Zero';
switch(model)
    case 1
        controlSubfolder = '20180409MembraneAssociationCD3ZetaMembraneOnEB0Zero';
    case 3
        controlSubfolder = '20181105MembraneAssociationCD3EpsilonMembraneOnEB0Zero';
end
...
switch (model)
    ...
    case 3
        %subfolder = '20180306CD3ZetaElectrostaticsEB0Sweep';
        subfolder = '20181105MembraneAssociationCD3EpsilonMembraneOnEB0Sweep';
            ...
            for e = 1:length(ER)
                for z = 1:length(ZR)
                    ...
                    if(exist(fullfile(folder,subfolder,filename))~=0)
                        ...
                        for i = 1:iSiteTotal
                            rMiSite(k,w,d,e,z,i) = M(1,7+9+5*iSiteTotal+3+bSiteTotal+2*iSiteTotal*bSiteTotal+iSiteTotal+(iSiteTotal+1)*NBINS+1+i);
                            rMiSiteNM(k,w,d,e,z,i) = rMiSite(k,w,d,e,z,i).*0.3;
                        end
                        
                        for i = 1:iSiteTotal
                            rM2iSite(k,w,d,e,z,i) = M(1,7+9+5*iSiteTotal+3+bSiteTotal+2*iSiteTotal*bSiteTotal+iSiteTotal+(iSiteTotal+1)*NBINS+1+iSiteTotal+i);
...
for i = 1:iSiteTotal
    rMiSiteControl(i) = M(1,7+9+5*iSiteTotal+3+bSiteTotal+2*iSiteTotal*bSiteTotal+iSiteTotal+(iSiteTotal+1)*NBINS+1+i);
    rMiSiteControlNM(i) = rMiSiteControl(i)*0.3;
end

for i = 1:iSiteTotal
    rM2iSiteControl(i) = M(1,7+9+5*iSiteTotal+3+bSiteTotal+2*iSiteTotal*bSiteTotal+iSiteTotal+(iSiteTotal+1)*NBINS+1+iSiteTotal+i);
    rM2iSiteControlNM(i) = rM2iSiteControl(i)*0.09;
end
...
plotData = reshape(rM2iSiteNM(k,w,:,e,z,3)-rMiSiteNM(k,w,:,e,z,3).^2,[length(PD),1]);
disp('Var Calc:');
disp(plotData);
rMVarTyr3 = plotData;
plot(PD,plotData,'-k','LineWidth',lw);
...
plotData = reshape(rM2iSiteNM(k,w,:,e,z,3)-rMiSiteNM(k,w,:,e,z,3).^2,[length(PD),1]);
rMVarTyr3 = plotData./(rM2iSiteControlNM(3)-rMiSiteControlNM(3).^2);
